...
{
    public class TOUService : BaseService<TOUFile>
    {
        public TOUService(IOutputService outputService) : base(outputService)
        {
        }

        /// <summary>
        /// This function process TOU files
        /// </summary>
        /// <param name="filesPath">directory path that contains TOU files</param>
        /// <returns></returns>
        public async Task ProcessFiles(string filesPath)
        {
            ...
            TransformBlock<string, OutputRecord> readAction = new TransformBlock<string, OutputRecord>(item =>
            {
                ...
                List<TOUFile> records = GetRecords(item);
        ...
        /// <returns></returns>
        public List<TOUFile> GetRecords(string file)
        {
            using (var reader = new StreamReader(file))
            {
                using (var csv = new CsvReader(reader))
                {
                    ...
                    return csv.GetRecords<TOUFile>().OrderBy(r => r.Energy).ToList();
                }
            }
        }
    }
